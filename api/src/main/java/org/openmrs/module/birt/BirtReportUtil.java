...
public class BirtReportUtil {
	...
	public static File getReportRepository() {
		...
		String reportDirectory = Context.getAdministrationService()
				.getGlobalProperty(BirtConfiguration.PROPERTY_REPORT_DIR);
	...
	public static String getReportDesignPath(String filename) {
		return new StringBuffer().append(BirtReportUtil.getReportRepository().getAbsolutePath())
				.append(BirtReportUtil.getReportRepository().getAbsolutePath().endsWith(BirtConstants.FILE_SEPARATOR)
						? "" : BirtConstants.FILE_SEPARATOR)
				.append(filename).append(".").append(BirtConstants.REPORT_DESIGN_EXTENSION).toString();
	}
	...
	public static String getDataExportPath(String filename) {
		// Copy the generated data export to the location expected by the report
		// module
		String datasetDir = Context.getAdministrationService()
				.getGlobalProperty(BirtConfiguration.PROPERTY_DATASET_DIR);

		// Create the filename for the final data export
		StringBuffer buffer = new StringBuffer();
		buffer.append(datasetDir);
		buffer.append(File.separatorChar); // TODO check to make sure the
	...
	/**
	 * Convenience method used to create an appropriate filename for a rendered
	 * report.
	 * 
	 * @param name
	...
	public static String getOutputFilename(String name, String format) {
		...
		String extension = (format != null) ? format.toLowerCase() : BirtConfiguration.DEFAULT_REPORT_OUTPUT_FORMAT;
		...
		StringBuffer buffer = new StringBuffer();
		buffer.append(getOutputDirectory().replace("/", File.separator)).append(File.separator)
				.append(name.replace(" ", "_")).append("-").append(BirtConstants.DATE_FORMATTER.format(new Date()))
	...
	public static File createDirectory(String directory) {
		...
		if (!folder.exists()) {
			...
			if (!folder.exists()) {
				throw new ModuleException(
						"The specified directory " + folder.getAbsolutePath() + " could not be created");
			}
		}
		// If folder does exist
			...
			if (!folder.isDirectory()) {
				throw new ModuleException(
						"The specified directory exists, but is not a directory at: " + folder.getAbsolutePath());
			}
		}
		return folder;
	}
	...
	/**
	 * Return an array of objects of the given type parsed from an array of
	 * strings.
	 * 
	 * @param type
	 * @param values
	 ...
	 */
	public static ParameterDefinition getParameter(IGetParameterDefinitionTask task, IScalarParameterDefn scalar,
			IReportRunnable reportRunnable, IParameterGroupDefn group) {
		...
		// Get report design and find default value, prompt text and data set
		// expression using the DE API
		...
		// Sets whether we should allow multiple values or not (based on
		// parameter type simple vs multi-value)
		parameter.setAllowMultiple("multi-value".equals(parameterHandle.getParamType()) ? true : false);

		// If the parameter's control type is not TEXT BOX, then it is some type
		// of SELECT LIST
