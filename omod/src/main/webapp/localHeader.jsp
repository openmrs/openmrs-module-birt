<ul id="menu">	
	...
	<openmrs:hasPrivilege privilege="Manage Reports">
		<li <c:if test='<%= request.getRequestURI().contains('<spring:message code="birt.reportLists"/>') %>'>class="active"</c:if>>
			<a href="${pageContext.request.contextPath}/module/birt/report.list"><spring:message code="birt.list.title"/>
			</a>
		</li>
		<li <c:if test='<%= request.getRequestURI().contains('<spring:message code="birt.reportForm"/>') %>'>class="active"</c:if>>
